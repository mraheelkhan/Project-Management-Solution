											...
											<div class="mailbox-attachment-info">
												<p class="mailbox-attachment-name">{{$asset->orginalfilename}}</p>
														...
														{{number_format((Storage::disk('local')->size('public/projects_assets/'.$projectDetail->id.'/messages/'.$asset->attachment)/1024),2)}} KBs
															<a href="/public{{Storage::disk('local')->url('public/projects_assets/'.$projectDetail->id.'/messages/'.$asset->attachment)}}" target="_blank" class="btn btn-default btn-xs pull-right"><i class="fa fa-cloud-download"></i></a>
														</span>
											</div>
										</li>
										@endif
...
$.ajax({
   url: "{{route('projectslinksdestroy', " + LinkID + " )}}",
   type: "POST",
